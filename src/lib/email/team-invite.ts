...
export async function sendTeamInviteEmail({
  ...
  if (!resend) {
    ...
    console.log("[email] Invite URL (dev):", inviteUrl);
  ...
  const { error } = await resend.emails.send({
    from: FROM_ADDRESS,
    to,
    subject: `You're invited to join ${orgName} on Agar`,
    html: buildInviteHtml({ inviteUrl, orgName, inviterName, role }),
  });
          ...
          <tr>
            <td style="padding:32px 40px 0;text-align:center">
              ...
              <p style="margin:12px 0 0;font-size:15px;color:#6b7280;line-height:1.6">
                <strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on Agar Smart Accounting as <strong>${role}</strong>.
              </p>
            </td>
          </tr>
          <tr>
