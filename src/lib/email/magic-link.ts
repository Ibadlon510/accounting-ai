...
export async function sendMagicLinkEmail({
  ...
  if (!resend) {
    ...
    console.log("[email] Magic link URL (dev):", magicUrl);
  ...
  const { error } = await resend.emails.send({
    from: FROM_ADDRESS,
    to,
    subject: "Sign in to Agar Smart Accounting",
    html: buildMagicLinkHtml(magicUrl, displayName),
  });
...
function buildMagicLinkHtml(magicUrl: string, name: string) {
  return `
<!DOCTYPE html>
<html lang="en">
          ...
          <tr>
            <td style="padding:32px 40px 0;text-align:center">
              ...
              <p style="margin:12px 0 0;font-size:15px;color:#6b7280;line-height:1.6">
                Hi <strong>${name}</strong>, click the button below to sign in to your Agar account. No password needed.
              </p>
            </td>
          </tr>
          <tr>
            ...
            <td style="padding:0 40px 32px;text-align:center">
              <div style="border-top:1px solid #f3f4f6;padding-top:20px">
                ...
                <p style="margin:8px 0 0;font-size:11px;color:#9ca3af">
                  &copy; ${new Date().getFullYear()} Agar Smart Accounting. All rights reserved.
                </p>
              </div>
            </td>
          </tr>
