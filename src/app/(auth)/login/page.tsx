...
import { Loader2, Sparkles, Mail, KeyRound } from "lucide-react";
...
function LoginContent() {
  ...
  useEffect(() => {
    if (magicToken && magicEmail && !magicLinkProcessed.current) {
  ...
  }, [magicToken, magicEmail, callbackUrl, router]);
  ...
  // Show loading state while auto-verifying magic link
  if (magicToken && magicEmail && !error) {
    return (
  ...
  return (
    <div>
      ...
      <div className="my-5 flex items-center gap-3">
        ...
        <span className="text-[11px] font-medium text-text-meta">OR</span>
        <div className="h-px flex-1 bg-border-subtle" />
      </div>

      {/* Mode Toggle: Password vs Magic Link */}
      <div className="mb-4 flex rounded-xl border border-border-subtle p-1">
        <button
          type="button"
      ...
      {mode === "magic-link" ? (
        magicLinkSent ? (
          <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-6 text-center">
            ...
            <p className="mt-3 text-[14px] font-medium text-green-800">Check your email</p>
            ...
            <button
              ...
              className="mt-4 text-[12px] font-medium text-green-700 underline hover:text-green-900"
            >
