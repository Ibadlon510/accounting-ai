...
export default function PurchaseCreditNotesPage() {
  ...
  function loadCreditNotes() {
    fetch("/api/accounting/credit-notes?type=purchase", { cache: "no-store" })
  ...
  useEffect(() => {
    ...
    fetch("/api/accounting/chart-of-accounts", { cache: "no-store" })
      ...
      .catch(() => {});
    fetch("/api/purchases/bills", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : { bills: [] }))
      .then((d) => setBills((d.bills ?? []).filter((b: Bill) => b.amountDue > 0)))
      .catch(() => {});
  }, []);
  ...
  async function handleCreate(data: {
    ...
    const res = await fetch("/api/accounting/credit-notes", {
      ...
      body: JSON.stringify({ ...data, creditNoteType: "purchase" }),
    });
  ...
  const filtered = creditNotes.filter(
    (cn) =>
      ...
      (cn.supplierName ?? "").toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="dashboard-card overflow-hidden !p-0">
        ...
        {filtered.map((cn) => (
          ...
          >
            ...
            <div className="col-span-3 text-right font-mono font-medium text-success">
