---
consolidatedFrom:
  - ai_accounting_suite_mvp_5e1f3dc5.plan.md
  - ai-accounting-saas-plan-fb14b0.md
  - final-implementation-plan-fb14b0.md
category: strategic
---

# Strategic Roadmap & MVP Plan

Consolidated from: AI Accounting Suite MVP, AI Accounting SaaS Plan, Final Implementation Plan.

---

## Section A: Technical Decisions & Stack

| Decision | Choice |
|----------|--------|
| Backend | Full-stack Next.js (TypeScript everywhere). Next.js 14+ App Router. |
| Hosting | Render |
| Multi-tenancy | Shared DB + Row-Level Security (RLS). `organization_id` on every table. |
...
| File Storage | AWS S3 |
| PDF | @react-pdf/renderer (generation); react-pdf + pdf.js (viewer for verify) |
| Email | Resend |
| AI (entry) | OpenAI GPT-4o-mini (structured outputs) — smart entry bar |
| AI (classify) | Rule-based TS engine + per-org learning — bank classification |
| AI (extraction) | Google Gemini 1.5 Flash (invoice/receipt extraction) |
| Payments | Stripe (post-MVP) |
| Repo | Single Next.js app (no turborepo) |
| Inventory costing | Both weighted average + FIFO (Phase 2, not MVP) |
| Design/brand | Finsera-inspired; design system in Main Plans |

**Per Strategic Roadmap and Sprint 1 plan.** No NestJS, no Prisma, no monorepo for MVP.
...
- **Document extraction**: Gemini 1.5 Flash for invoice/receipt extraction
...
### MVP Additions (Document Vault)
- **documents** — id, organization_id, s3_key, status, ai_confidence, extracted_data, created_at
- **transactions** (document-derived) — id, document_id, date, total_amount, vat_amount, net_amount, currency, merchant_name, gl_account_id, organization_id, timestamps
- **merchant_maps** — id, organization_id, merchant_name, gl_account_id, confidence, last_used, unique on (organization_id, merchant_name)

**Note:** Main Plans list `vat_return_lines`; current Drizzle schema has `vat_returns`. Add `vat_return_lines` (or equivalent box-level storage) when implementing VAT 201 if needed. All new tables must include `organization_id` and be covered by RLS.

---

## Section D: Sprint Roadmap (9 sprints, ~20 weeks)
...
- Base UI shell (Finsera design system, sidebar nav, layout, dark/light mode)
- Organization settings
- See [02-foundation-and-sprint1-implementation.md](02-foundation-and-sprint1-implementation.md) for detailed Sprint 1 spec
...
### Sprint 4: Purchases (Week 9–10) + MVP Phase 1
- Supplier management
- Bills entry + expense recording
- Input VAT tracking
- Supplier payment recording
- **MVP Phase 1 (Document Vault):** documents/transactions/merchant_maps schema, S3 vault upload, document list, manual verify
...
### Sprint 8: AI + Reports + Polish (Week 16–18) + MVP Phases 2–3
...
- **MVP Phase 2:** Gemini extraction, token economy, split-screen verify
- **MVP Phase 3:** VAT 201 API + UI, Archive mode
...
- Security hardening
- Deploy to Render
- Include document vault and token UX in hardening

---

## Section E: Current State vs Blueprint

| Blueprint | Current codebase |
|-----------|------------------|
| **Stack** | Next.js 16, App Router, Tailwind, Shadcn, Zustand, Zod, Drizzle, PostgreSQL (Render), NextAuth.js. No NestJS, no Prisma, no monorepo. |
...
| **Auth** | NextAuth.js (login, signup, middleware). Onboarding has TODO: "Create organization in database via server action". |
...
| **VAT** | vat/page.tsx and vat-data mock; vatReturns table exists but no FTA VAT 201 Box 1a–15 mapping. |
...
#### 1.1 Foundation
- **Onboarding → DB:** Replace the TODO in onboarding/page.tsx with a server action (or API route) that: creates a row in `organizations` (and `user_roles` linking the current user), using NextAuth session `user.id` and existing Drizzle schema. Ensure dashboard/workspaces resolve current org from session (e.g. first org or selected org in cookie/header).
- **Tenant fields:** In schema.ts, add to `organizations`: `subscriptionPlan` (e.g. `varchar('subscription_plan', { length: 20 }).default('FREELANCER')`), `tokenBalance` (`integer('token_balance').default(50)`). Add a `Plan` const/enum in code for FREELANCER / BUSINESS / ENTERPRISE if you want type safety.
- **Workspaces page:** Replace hardcoded `organizations` in workspaces/page.tsx with data from DB (server component or fetch from API) and persist "current workspace" (e.g. cookie or user preference table) so all document/transaction flows are org-scoped.

#### 1.2 Document Vault schema (Drizzle)
Add to schema.ts (align with blueprint; keep naming consistent for later AI):
...
## Section G: Resolved Decisions & Sprint–Phase Map

### Resolved decisions (no conflicts)

| Topic | Main Plans | This MVP (Zero-Entry / Vault) | Resolution |
|-------|------------|-------------------------------|------------|
| **Stack** | Next.js 14+, Drizzle, Render PostgreSQL, Tailwind, shadcn, Zustand, TanStack Query | Same | No change. |
| **Auth & multi-tenancy** | NextAuth.js, `organization_id` on every table | Same; add current-org resolution (cookie/preference) | Use org from session for all new document/report APIs. |
...
| **File storage** | AWS S3 | AWS S3 me-central-1 for Document Vault | **AWS S3** for all file storage: general app files and compliance Document Vault (upload → temp, verify → retention-vault with Object Lock). |
...
- **File structure:** Add under `/lib/ai/`: `extract-invoice.ts` (Gemini), `schemas.ts` (Zod for extraction); keep existing `classifier.ts`, `nl-parser.ts`, `rules-engine.ts` for NL entry and bank classification. Add `/lib/vat/` if not present: `calculator.ts`, `return-builder.ts`; extend for VAT 201 box mapping in Phase 3.
...
### Deferred to Phase 2+ (full list)
- Stripe subscription billing
- FIFO costing option
- Arabic UI
- OCR receipt scanning (replaced by Gemini extraction in MVP)
...
| Area | File(s) |
|------|---------|
| Schema | accounting-ai/src/lib/db/schema.ts – add documents, transactions, merchant_maps; org token + plan |
| Onboarding | accounting-ai/src/app/(auth)/onboarding/page.tsx – server action + org creation |
...
| VAT 201 | New: app/api/reports/vat-201/route.ts, extend vat/page.tsx |
