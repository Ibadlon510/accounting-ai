---
name: Document Verify Workflow Analysis
overview: The document verify page does NOT route by document type. Bank statement import (CSV, Excel, PDF, image) is not implemented. There is no workflow visibility—users cannot see what will happen after they verify.
todos: []
isProject: false
---
...
**File:** [src/app/api/documents/[id]/process/route.ts](src/app/api/documents/[id]/process/route.ts)

- Calls `extractInvoiceFromImage()` in [src/lib/ai/extract-invoice.ts](src/lib/ai/extract-invoice.ts)
...
- `InvoiceExtractionSchema` in [src/lib/ai/schemas.ts](src/lib/ai/schemas.ts) has no `document_type` field
- Stored in `documents.extractedData` as JSONB

### 2. Verify Page UI

**File:** [src/app/(dashboard)/documents/[id]/verify/page.tsx](src/app/(dashboard)/documents/[id]/verify/page.tsx)
...
**File:** [src/app/api/documents/[id]/verify/route.ts](src/app/api/documents/[id]/verify/route.ts)
...
The right-hand verification column must **mirror the creation form** for each document type. Reference components: [CreateInvoicePanel](src/components/modals/create-invoice-panel.tsx), [CreateBillPanel](src/components/modals/create-bill-panel.tsx).
...
**Reference:** [CreateBillPanel](src/components/modals/create-bill-panel.tsx) line-item table; add a GL Account column per line for expense allocation.
...
3. **Verify page branching** — Each document type shows a verification form that **mimics the creation form** for that entity (see [Verification Form Structure](#verification-form-structure-by-document-type) below)
...
**Relevant code:**
- [src/app/api/documents/upload/route.ts](src/app/api/documents/upload/route.ts) — `ALLOWED_TYPES`: PDF, JPEG, PNG, WebP
- [src/app/(dashboard)/banking/page.tsx](src/app/(dashboard)/banking/page.tsx) — "Import CSV" calls `comingSoon("CSV Import")`
- [src/components/workspace/smart-drop-zone.tsx](src/components/workspace/smart-drop-zone.tsx) — `accept=".pdf,.jpg,.jpeg,.png,.webp"`
